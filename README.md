...
6. If you would like to run experiments using GRU, MLP or Tfenc model, please modify Configuration file based on `train.yaml` and `predict.yaml`.
7. `data-module` determines the input preprocessing strategy, this term can be chosen from `e2e_data_MLP`, `e2e_data_hav`, and `e2e_data_hit`. If `e2e_data_hit` is used, please specify `hit_input` to be true, and don't forget to set the `encoder_params`'s `input_size` to be twice of embedding_dim. Also, please specify a new `vocab_path` for each different input type.
8. `model-module` determines the model structure, this term can be chosen from `e2e_model_transformer`, `e2e_model_MLP`, `e2e_model_gru`, and `e2e_model_tfenc`. 
